...
def fetch_gen9_pokemon_from_api():
    ...
    for pokemon_id in range(906, 1026):  # Gen 9 Pokémon IDs
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                abilities = [
                    {'name': ab['ability']['name'], 'is_hidden': ab['is_hidden'], 'slot': ab['slot']}
...
def save_to_json(pokemon_data, json_filename="pokeJsons/gen9output.json"):
...
def get_gen9_pokemon_data(json_filename="pokeJsons/gen9output.json"):
    if os.path.exists(json_filename):
        try:
            ...
            data = fetch_gen9_pokemon_from_api()
...
def regenerate_json_file(json_filename="pokeJsons/gen9output.json"):
